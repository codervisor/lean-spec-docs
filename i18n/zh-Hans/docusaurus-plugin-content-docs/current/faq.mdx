...
LeanSpec 是一个为 AI 辅助开发优化的轻量级规范框架。它是一个 CLI 工具 + markdown 文件，位于您的仓库中，帮助您记录技术决策并指导 AI 编码工具，同时将规范保持在约 2,000 令牌的最佳范围内。
...
三个原因：
1. **人类注意力范围** - 人与 AI 搭档一次只能处理约 2-3K 令牌
2. **AI 性能** - 上下文质量在 50K 令牌硬上限之前就开始下降
3. **工作记忆** - 令牌数量直接对应认知负荷，而行数受格式影响很大

如果您的规范超过 3,500 令牌，请将其拆分为子规范文件（`DESIGN.md`、`IMPLEMENTATION.md`、`TESTING.md` 等）；超过 5,000 令牌必须立即拆分。详见[第一原则](/zh-Hans/docs/advanced/first-principles#1-上下文经济context-economy---适应工作记忆)。
...
**预防：** 将规范控制在 2,000 令牌以内（3,500 令牌警告，5,000 令牌硬限制），并在编辑前运行 `lean-spec tokens <spec>` 检查。
