...
**查看Spec：**
- `lean-spec view <spec>` - 查看Spec（格式化）
- `lean-spec view <spec> --raw` - 获取原始 markdown
- `lean-spec view <spec> --json` - 获取结构化 JSON
- `lean-spec open <spec>` - 在编辑器中打开Spec
- `lean-spec files <spec>` - 列出Spec中的所有文件
...
创建或更新Spec时，包括 YAML frontmatter：
...
**重要**：始终使用 `lean-spec update` 修改状态、优先级、标签和负责人。永远不要手动编辑这些系统管理字段的frontmatter。

## SDD 工作流程

1. **发现** - 使用 `lean-spec list` 检查现有Spec
2. **计划** - 需要时使用 `lean-spec create <name>` 创建Spec
...
- Spec与实现保持同步
- 完成工作前始终验证：
  - 运行 `lean-spec validate` 检查Spec结构
  - 标记完成前修复任何验证错误
...
- [编写 AI 可执行的Spec](/zh-Hans/docs/advanced/ai-assisted-spec-writing) - 12 个实用模式
