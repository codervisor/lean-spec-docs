...
**源**：平面 markdown 文件（例如，`docs/adr/####-title.md`）
...
- [ ] 测试规格发现：`lean-spec list`、`lean-spec search`
...
**解决方案**：手动设置时间戳或使用当前日期
```bash
# backfill 将为没有 git 历史的规格使用当前时间戳
lean-spec backfill --assignee --all
...
你不需要重写内容来匹配特定结构。LeanSpec 的首要原则是"意图重于实现"——只要规格清晰地传达意图，确切的格式并不重要。

### 问题：迁移后关系损坏

**解决方案**：手动更新 `depends_on` 和 `related` 字段
```bash
# 为每个规格编辑 README.md 中的 frontmatter
---
depends_on: [002, 005]
related: [010, 012]
---
...
**注意**：时间包括 backfill 和验证。复杂的迁移可能需要手动调整。
...
1. **学习工作流**：[创建和管理规格](/docs/guide/usage/cli/creating-managing)
