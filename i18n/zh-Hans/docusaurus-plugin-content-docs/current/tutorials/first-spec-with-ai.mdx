...
<Tabs groupId="ai-tool">
  <TabItem value="manual" label="手动设置" default>
    ```bash
  ...
  <TabItem value="copilot" label="GitHub Copilot CLI">
    ```bash
    gh copilot suggest "从 lean-spec 搭建 dark-theme 示例并进行设置"
    ```
    
    或使用自然语言：
    ```bash
    ?? 从 lean-spec 搭建 dark-theme 示例
    ```
  </TabItem>
  <TabItem value="claude" label="Claude Code">
    在终端中使用 Claude Code：
    ```bash
    claude "搭建 lean-spec 的 dark-theme 示例：npx lean-spec init --example dark-theme，然后进入目录，安装依赖项并启动开发服务器"
    ```
  </TabItem>
  <TabItem value="gemini" label="Gemini CLI">
    ```bash
    gemini "为我设置 LeanSpec dark-theme 示例项目"
    ```
  </TabItem>
</Tabs>
