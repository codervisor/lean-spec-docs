...
<Tabs groupId="ai-tool">
  <TabItem value="copilot" label="GitHub Copilot CLI">
    ...
    # 向 Copilot CLI 发送提示
    copilot -p "使用 LeanSpec 帮我重构这个应用，提取一个可重用的 API 客户端" \
      --files specs/ \
      --add-dir .
    ```
  </TabItem>
  <TabItem value="claude" label="Claude Code">
    ...
    # 向 Claude Code 发送提示
    claude prompt \
      "使用 LeanSpec 帮我重构这个应用，提取一个可重用的 API 客户端" \
      --include-files specs/ \
      --include-dir .
    ```
  </TabItem>
  <TabItem value="gemini" label="Gemini CLI">
    在终端中使用 Gemini CLI：
    ```bash
    gemini -p "使用 LeanSpec 帮我重构这个应用，提取一个可重用的 API 客户端"
    ```
  </TabItem>
  <TabItem value="ide" label="IDE" default>
    ...
    1. 在 IDE 中打开项目
    2. 打开 AI 助手聊天面板
    3. 发送提示：_"使用 LeanSpec 帮我重构这个应用，提取一个可重用的 API 客户端"_
  </TabItem>
</Tabs>
