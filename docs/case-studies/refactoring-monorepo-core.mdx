...
**Spec**: [067-monorepo-core-extraction](https://github.com/codervisor/lean-spec/tree/main/specs/067-monorepo-core-extraction)  
**Status**: ✅ Complete  
**Complexity**: High  
**Duration**: 1 week
...
│   │   ├── types/            # Platform-agnostic types
│   │   ├── parsers/          # Spec parsing
│   │   ├── validators/       # Validation logic
│   │   └── utils/            # Stats, filters
│   │
│   ├── cli/                  # 🔧 EXISTING: CLI/MCP
│   │   ├── commands/
│   │   └── adapters/
│   │       └── fs-storage.ts # File system adapter
│   │
│   └── web/                  # 🌐 FUTURE: Web app
│       └── adapters/
│           └── github-storage.ts  # GitHub adapter
```
...
[View full spec →](https://github.com/codervisor/lean-spec/blob/main/specs/067-monorepo-core-extraction/README.md)
...
{
  "dependencies": {
    "@leanspec/core": "workspace:*"  // ❌ Breaks published package
  }
}
```
...
- ✅ Explicit migration plan prevented missing edge cases
- ✅ Test strategy ensured no regressions
- ✅ Build configuration documented prevented publish issues
- ✅ Rollback plan (kept in spec) gave confidence

**What could have gone wrong**:
- Publishing with `workspace:*` dependencies → Would break users
- Missing sync-to-async conversion → Would break web app later
- No test updates → Would have failing CI
- Breaking CLI without migration guide → Would strand users
...
✅ **Phased implementation**
   - One package at a time (core → cli → web)
...
1. **Phased plan** - Clear steps (setup → extract → adapt → test)
...
- ✅ Include code examples for adapters
- ✅ Document non-obvious configuration

**Don't**:
- ❌ Expect AI to make architectural decisions
- ❌ Assume AI understands build tool nuances
- ❌ Leave judgment calls implicit
...
**Template**: Use [spec 067](https://github.com/codervisor/lean-spec/tree/main/specs/067-monorepo-core-extraction) as a template for breaking refactors that need careful coordination.
